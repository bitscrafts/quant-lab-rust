...
pub fn load_transactions(path: impl AsRef<Path>) -> Result<Vec<Transaction>, CommonError> {
    ...
    for (idx, result) in reader.records().enumerate() {
        let record =
            result.map_err(|e| CommonError::ParseError(format!("Row {}: {}", idx + 2, e)))?;
        ...
        if num_fields < 3 {
            return Err(CommonError::ParseError(format!(
                "Row {}: insufficient columns (need at least 3)",
                idx + 2
            )));
        }
        ...
        for i in 1..(num_fields - 2) {
            let value = record
                .get(i)
                .ok_or_else(|| {
                    CommonError::ParseError(format!("Row {}: missing field {}", idx + 2, i))
                })?
                .parse::<f64>()
                .map_err(|e| {
                    CommonError::ParseError(format!("Row {}, field {}: {}", idx + 2, i, e))
                })?;
            features.push(value);
        }
        ...
        let amount = record
            .get(num_fields - 2)
            .ok_or_else(|| CommonError::ParseError(format!("Row {}: missing Amount", idx + 2)))?
            .parse::<f64>()
            .map_err(|e| CommonError::ParseError(format!("Row {}, Amount: {}", idx + 2, e)))?;

        // Class is last column
        let class = record
            .get(num_fields - 1)
            .ok_or_else(|| CommonError::ParseError(format!("Row {}: missing Class", idx + 2)))?
            .parse::<u8>()
            .map_err(|e| CommonError::ParseError(format!("Row {}, Class: {}", idx + 2, e)))?;
