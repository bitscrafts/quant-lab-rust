...
mod tests {
    ...
    fn test_ff3_single_factor_reduces_to_capm() {
        ...
        let mkt = [0.005, 0.015, -0.005, 0.02, 0.005, 0.025, -0.01, 0.01, 0.02, -0.002];
        let smb = [0.001, -0.002, 0.0015, -0.001, 0.002, -0.0015, 0.001, -0.002, 0.0015, -0.001];
        let hml = [0.0015, 0.001, -0.002, -0.0015, 0.001, 0.002, -0.001, 0.0015, -0.002, 0.001];
        ...
        assert!((ff.beta_mkt - beta_mkt_true).abs() < 1e-6, "beta_mkt={} true={}", ff.beta_mkt, beta_mkt_true);
        assert!((ff.alpha - alpha_true).abs() < 1e-6, "alpha={} true={}", ff.alpha, alpha_true);
        // The SMB and HML betas should be zero (asset has no SMB/HML component).
        assert!(ff.beta_smb.abs() < 1e-6, "beta_smb should be 0, got {}", ff.beta_smb);
        assert!(ff.beta_hml.abs() < 1e-6, "beta_hml should be 0, got {}", ff.beta_hml);
    ...
    fn test_ff3_alpha_zero_when_factor_generated() {
        ...
        let mkt = [0.01, -0.02, 0.03, 0.0, -0.01, 0.02, -0.005, 0.015, -0.01, 0.025];
        let smb = [0.005, 0.01, -0.005, 0.02, -0.01, 0.0, 0.015, -0.005, 0.01, -0.015];
        let hml = [-0.002, 0.003, 0.001, -0.001, 0.002, -0.003, 0.0, 0.002, -0.001, 0.003];
        let alpha_true = 0.001_f64;
        let b_m = 1.1_f64;
        let b_s = 0.5_f64;
        let b_h = 0.3_f64;
