...
\begin{keyinsight}
This is not throwaway code. Every component we build follows a \textbf{library-first design}---composable, testable, and reusable across future projects.
\end{keyinsight}
...
impl ZScoreDetector {
    ...
    pub fn fit(&mut self, transactions: &[Transaction]) {
        ...
        // Initialise mean/std accumulators to zero
...
\begin{keyinsight}
This implementation will evolve into \crate{quant-lab}. Before writing any struct, we ask: \textbf{what trait should this implement?}
\end{keyinsight}
...
While this chapter uses concrete types, later chapters extract traits:
